...
public class FDSService implements Serializable {
    ...
    public BufferedReader getReader(String filePath) throws FDSException {
        try (
                InputStream inputStream = FileSystem.get(new URI(filePath), new Configuration()).open(new Path(filePath));
    ...
    public BufferedWriter getWriter(String filePath) throws URISyntaxException, FDSException {
        try (
                OutputStreamWriter outputStreamWriter = new OutputStreamWriter(FileSystem.get(new URI(filePath), new Configuration()).create(new Path(filePath)));
