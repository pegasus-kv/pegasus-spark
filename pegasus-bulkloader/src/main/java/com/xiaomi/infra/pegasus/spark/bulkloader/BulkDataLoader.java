...
public class BulkDataLoader {
  ...
  public BulkDataLoader(
      Config config, Iterator<Tuple2<RocksDBRecord, String>> iterator, int partitionId) {
    ...
    this.bulkLoadInfo =
        new BulkLoadInfo(
            config.DBCluster, config.DBTableName, config.DBTableId, config.DBTablePartitionCount);
    this.dataMetaInfo = new DataMetaInfo();

    this.fdsService = new FDSService();
    this.sstWriter =
        new SSTWriter(new RocksDBOptions(config), config.DBTablePartitionCount, partitionId);
  }
